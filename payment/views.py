...
def complete_order(request):
    ...
    # Get total prices of itens

    total_cost = cart.get_total()

    # Create order -> Account user with/without shipping information
    ...
    for item in cart:
        OrderItem.objects.create(order_id=order.id, product=item['product'], quantity=item['qty'], price=item['price'], user=request.user)

    # create order = Guest user without an account
